...
export function Sidebar3D({ className, mobileOpen, onMobileClose }: SidebarProps) {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      // Function to get role and update nav items
      const updateRoleAndNavItems = () => {
        try {
          ...
          const roleItems = roleSpecificNavItems[role as keyof typeof roleSpecificNavItems] || []
          setNavItemsState([...commonNavItems, ...roleItems])
        } catch (error) {
          console.error("Error reading userRole from localStorage:", error)
          setNavItemsState([...commonNavItems])
        }
      }

      // Initial update
      updateRoleAndNavItems()
  ...
  const toggleSidebar = () => {
    ...
    const event = new CustomEvent("sidebar-toggle", { detail: { collapsed: !collapsed } })
  ...
  return (
    <>
      ...
      <div className={sidebarVisibilityClasses}>
              ...
              return (
                <Link key={item.name} href={item.href} onClick={isMobile ? onMobileClose : undefined}>
                  ...
                  >
                    ...
                    {(!collapsed || isMobile) && <span className="ml-3 transition-all duration-200">{item.name}</span>}
                  </motion.div>
                </Link>
              )
            })}
        ...
        {/* Help section */}
        <div className={cn("px-2 py-2", collapsed && !isMobile ? "hidden" : "block")}>
          ...
          <div className={cn("flex items-center", collapsed && !isMobile ? "justify-center" : "justify-between")}>
            <div className="flex items-center gap-3">
              <Avatar className="h-10 w-10 border-2 border-primary/20">
                <AvatarImage src={profile?.profile_image || undefined} alt={userInitials} />
